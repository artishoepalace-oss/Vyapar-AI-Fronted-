...
(function(){
  ...
  /* Account subscription identity is owned by assets/scripts/plan-badge-menu-645.js.
     Do not add another tick here. */
  function removeDuplicatePlanMark(){
    document.querySelectorAll('.vy647-plan-mark').forEach(function(mark){mark.remove()});
  }

  ensureSplash();hideLegacyLoader();waitMinimum(routeAfterSplash);
  new MutationObserver(function(){hideLegacyLoader();removeDuplicatePlanMark()}).observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',function(){hideLegacyLoader();removeDuplicatePlanMark()},{once:true});
  window.addEventListener('load',function(){hideLegacyLoader();removeDuplicatePlanMark()},{once:true});
})();
