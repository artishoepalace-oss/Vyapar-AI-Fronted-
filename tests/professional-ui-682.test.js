...
const webIndex=read('web','index.html'),androidIndex=read('android-app','app','src','main','assets','index.html'),finalCss=read('frontend-source','android','styles','flat-black-ios-861.css'),bundle=read('android-app','app','src','main','assets','assets','styles','vyapar-ui.css'),version=JSON.parse(read('version.json')),cacheKey=String(version.versionCode)+'-workspace1';
...
assert(androidIndex.includes(`vyapar-ui.css?v=${cacheKey}`),'Android UI bundle must use current cache key');
