...
const activity = read(path.join(root, 'android-app/app/src/main/java/com/vyaparai/app/MainActivity.java'));
...
assert.match(activity, /new UpdateManager\(this, webView\)/);
assert.match(activity, /checkForAppUpdate\(boolean manual\)/);
assert.match(activity, /downloadAndInstallUpdate\(String manifestJson\)/);
assert.match(activity, /updateManager\.onResume\(\)/);
