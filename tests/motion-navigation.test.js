...
test('Release identity and bundled motion order are synchronized',()=>{
 assert.equal(JSON.parse(fs.readFileSync(path.join(root,'version.json'))).versionName,'20.10.2004.00015.2026');
 for(const base of ['web','android-app/app/src/main/assets']){
  const html=fs.readFileSync(path.join(root,base,'index.html'),'utf8');assert(html.includes('vyapar-ui.css?v=2010200415-workspace1'));assert(!html.includes('motion-20102004.css'));assert(!html.includes('surface-hierarchy-20102004.css'));
