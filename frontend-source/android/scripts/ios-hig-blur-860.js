...
(function(){
  ...
  root.classList.add('vy860-hig-ios');
  ...
  function decorateNav(){
    document.querySelectorAll('.nav button[data-tab]').forEach(function(button){
      if(!button.dataset.androidTab) button.dataset.androidTab=button.dataset.tab;
  ...
  function decorateMore(){
    ...
    if(!sheet) return;
    const settingsItem=sheet.querySelector('.android-sheet-item[data-tab="settings"]');
    if(settingsItem) settingsItem.classList.add('is-full');
  }

  function cleanupLegacyLiquidBits(){
    document.querySelectorAll('.android-nav-glass-indicator').forEach(function(node){ node.remove(); });
  }

  const previousSetTab=typeof window.setTab==='function' ? window.setTab : null;
  if(previousSetTab && !previousSetTab.__vy860Wrapped){
    const wrapped=function(){
      const result=previousSetTab.apply(this, arguments);
      requestAnimationFrame(function(){ decorateNav(); cleanupLegacyLiquidBits(); decorateMore(); });
      return result;
    };
    wrapped.__vy860Wrapped=true;
    window.setTab=wrapped;
  }

  const previousOpenMore=typeof window.openMoreSheet==='function' ? window.openMoreSheet : null;
  if(previousOpenMore && !previousOpenMore.__vy860Wrapped){
    const wrappedOpen=function(){
      const result=previousOpenMore.apply(this, arguments);
      requestAnimationFrame(function(){ decorateMore(); cleanupLegacyLiquidBits(); });
      return result;
    };
    wrappedOpen.__vy860Wrapped=true;
    window.openMoreSheet=wrappedOpen;
  }

  const observer=new MutationObserver(function(){
    decorateNav();
    decorateMore();
    cleanupLegacyLiquidBits();
  });
