 ...
 try{
  ...
  for(const width of (process.env.QA_WIDTHS||'320,360,412').split(',').map(Number)){
   ...
   try{
    ...
    await go('subscription');assert((await page.locator('.subscription-intro').innerText()).includes('Your Business plan is active'));assert.equal(await page.locator('.subscription-status').count(),0,'Duplicate current-plan status strip is removed from the DOM with no reserved space');assert.equal(await page.locator('.subscription-business-card.active-plan').count(),1);await shot('plans');
