...
(async()=>{
 ...
 for(const width of widths){
  ...
  async function geometry(expected){
    ...
    assert.equal(g.color,g.topColor,'Header and navbar share the same black');assert.equal(g.selectedColor,'rgb(128, 1, 31)');assert.equal(g.selectedGlow,'none','Navbar labels have no outer shadow');assert(g.rim.includes('inset'),'Glossy rim is inset and preserves geometry');
    assert.equal(g.selectedIcon,'rgb(128, 1, 31)','Selected icon matches label');assert.equal(g.iconGlow,'none','Navbar icons have no outer shadow');
